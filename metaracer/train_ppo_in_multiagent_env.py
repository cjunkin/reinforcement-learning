...
from agents.chrispark_ppo_agent.ppo_trainer import PPOTrainer, PPOConfig
...
environment_config = dict(
    ...
    # Reward setting
    crash_sidewalk_penalty = 50,  # default = 50
    success_reward = 100,  # default = 100
    speed_reward = 30,  # multiplier
    driving_reward = 1,
    idle_penalty = 100,
    out_of_road_penalty = 75, 
    crash_vehicle_penalty = 1,  # Default is 0 (10 for test). Set to -10 so env will reward agent +10 for crashing.

    # # Reward setting
    # crash_sidewalk_penalty = 75,  # default = 50
    # success_reward = 150,  # default = 100
    # speed_reward = 7,  # default = 0
    # driving_reward = 2,
    # idle_penalty = 100,
    # out_of_road_penalty = 100, 
    # crash_vehicle_penalty = 0,  # Default is 0 (10 for test). Set to -10 so env will reward agent +10 for crashing.
...
parser.add_argument(
    "--opponent-agent-name",
    default="chrispark_ppo_agent_v2",
    type=str,
...
class RacingEnvWithOpponent(MultiAgentRacingEnv):
    ...
    # [Normalizing reward functions]
    def update_running_stats(self, reward):
        self.num_rewards += 1
        delta = reward - self.running_mean
        self.running_mean += delta / self.num_rewards
        delta2 = reward - self.running_mean
        self.running_variance += delta * delta2

    def normalize_reward(self, reward):
        if self.num_rewards > 1:
            running_std = (self.running_variance / (self.num_rewards - 1)) ** 0.5
            return (reward - self.running_mean) / running_std
        else:
            return reward
    
    def scale_reward(self, normalized_reward, scale_min=-10, scale_max=10):
        range_width = scale_max - scale_min
        scaled_reward = normalized_reward * (range_width / 2)
...
if __name__ == '__main__':
    ...
    hyperparams = pd.DataFrame([{
        "crash_sidewalk_penalty": environment_config["crash_sidewalk_penalty"],
        "success_reward": environment_config["success_reward"],
        "speed_reward": environment_config["speed_reward"],
        # "driving_reward": environment_config["driving_reward"],
        # "idle_penalty": environment_config["idle_penalty"],
        # "out_of_road_penalty": environment_config["out_of_road_penalty"],
        # "crash_vehicle_penalty": environment_config["crash_vehicle_penalty"]
    }])
