...
class PPOTrainer:
    ...
    def compute_action(self, obs, deterministic=False):
        ...
        if self.discrete:  # Please use categorical distribution.
            ...
            actions = dist.sample().view(-1, self.num_actions)            
    ...
    def compute_loss(self, sample):
        ...
        # [DONE]: Implement policy loss
        # The importance sampling factor, the ratio of new policy prob over old policy prob
        ratio = torch.exp(action_log_probs - old_action_log_probs_batch)
        policy_loss = -torch.min( ratio * adv_targ , torch.clamp(ratio, 1 - self.clip_param, 1 + self.clip_param) * adv_targ )

        policy_loss_mean = policy_loss.mean()

        # [DONE]: Implement value loss
